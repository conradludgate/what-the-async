...
impl Park for Parker {
    ...
    fn park_timeout(&mut self, duration: Duration) {
        ...
        match self.inner.shared.driver.try_lock() {
            Ok(mut driver) => driver.park_timeout(duration),
            Err(std::sync::TryLockError::Poisoned(driver)) => {
                driver.into_inner().park_timeout(duration);
            }
...
impl Inner {
    ...
    fn park(&self) {
        ...
        match self.shared.driver.try_lock() {
            Ok(mut driver) => self.park_driver(&mut driver),
            Err(std::sync::TryLockError::Poisoned(driver)) => {
                self.park_driver(&mut driver.into_inner());
            }
            Err(std::sync::TryLockError::WouldBlock) => self.park_condvar(),
        }
    }
